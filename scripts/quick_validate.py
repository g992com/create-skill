...
def _parse_simple_frontmatter(frontmatter_text: str) -> Optional[dict]:
    ...
    for raw_line in frontmatter_text.splitlines():
        ...
        if not stripped or stripped.startswith("#"):
            continue
        is_indented = raw_line[:1].isspace() if raw_line else False
...
def validate_skill(skill_path):
    ...
    if yaml is not None:
        ...
        if frontmatter is None:
            return False, "Invalid YAML in frontmatter (install PyYAML for full support)"
    allowed_properties = {"name", "description", "license", "allowed-tools", "metadata"}
    unexpected_keys = set(frontmatter.keys()) - allowed_properties
    if unexpected_keys:
        return False, f"Unexpected key(s) in frontmatter: {', '.join(sorted(unexpected_keys))}. Allowed: {', '.join(sorted(allowed_properties))}"
